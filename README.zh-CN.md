...
平台骨架完全可用，包含研究、回测、执行、风控和 Agent 协作模块。

### 近期计划（1-3 个月）

- 领域契约：统一研究、数据科学、实验和生命周期共享类型
- 数据科学平台：数据集注册、数据质量检查、特征血缘
- Research OS：研究工作区生命周期、实验注册、模型注册
- 回测实验室：可复现规格、成本模型、市场环境归因、鲁棒性诊断

### 中期计划（3-6 个月）

- 策略生命周期：基于证据的 promotion gate（paper → live）
- 风控控制面：策略引擎、交易前评估、紧急熔断
- 执行控制面：订单生命周期、broker 适配层、异常恢复
- 计算平台：任务调度、产物管理、异步任务运行器

### 长期愿景（6-12 个月）

- AI 研究平台：受治理 Agent 工具、审查工作流、证据引用
- 机构运营：多账户、细粒度权限、合规审计
- 开放生态：数据/broker/模型连接器、策略包市场
- 可观测性与加固：结构化事件、完整性检查、部署治理
