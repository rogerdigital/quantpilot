...
function RiskPage() {
  ...
  const [riskActionResult, setRiskActionResult] = useState<
    RiskPolicyActionResponse['action'] | null
  >(null);
  const {
    data: workbench,
    loading,
    error,
  } = useRiskWorkbench(`${state.controlPlane.lastSyncAt}:${riskActionRefreshKey}`);
  ...
  const liveEquity = Number(
    brokerSnapshot?.account?.equity || workbench.summary.liveEquity || live.nav
  );
  const liveExposure = workbench.summary.liveExposurePct || live.exposure;
  const selectedRiskEvent =
    workbench.recent.riskEvents.find((item) => item.id === selectedRiskEventId) ||
    workbench.recent.riskEvents[0] ||
    null;
  const selectedSchedulerTick =
    workbench.recent.schedulerTicks.find((item) => item.id === selectedSchedulerTickId) ||
    workbench.recent.schedulerTicks[0] ||
    null;
  const riskIncidentCount = workbench.reviewQueue.incidents.length;
  const selectedLaneStatus = useMemo(
    () =>
      workbench.lanes.find((item) => item.key === 'risk-events')?.status ||
  ...
  function focusRiskRunbook(key: RiskRunbookActionKey) {
    if (key === 'review-risk-off') {
      const riskOff =
        workbench.recent.riskEvents.find((item) => item.status === 'risk-off') ||
        workbench.reviewQueue.riskEvents[0];
    ...
    if (key === 'check-compliance-alerts') {
      const compliance = workbench.recent.riskEvents.find(
        (item) =>
    ...
    if (key === 'release-emergency-brake') {
      const blocked =
        workbench.reviewQueue.executionPlans.find(
          (item) => item.riskStatus === 'blocked' || item.status === 'blocked'
        ) || workbench.reviewQueue.executionPlans[0];
    ...
    <>
        ...
        items={[
          {
            ...
            value: workbench.posture.title || state.riskLevel,
            accent: true,
          },
          {
            label: copy[locale].labels.marketState,
            value: workbench.summary.brokerConnected
              ? copy[locale].labels.connected
              : copy[locale].labels.fallback,
          },
          {
            label: copy[locale].terms.pendingApprovals,
      ...
      <section className="metrics-grid">
        <article className="metric-tile">
          <div className="tile-label">{locale === 'zh' ? '组合风险' : 'Portfolio Risk'}</div>
          <div className={`tile-value tone-${riskTone(selectedLaneStatus)}`}>
            {workbench.posture.status}
          </div>
          <div className="tile-sub">
            {workbench.posture.detail ||
              (locale === 'zh' ? '当前系统风控结论' : 'Current system risk posture')}
          </div>
        </article>
        <article className="metric-tile">
        ...
        <article className="metric-tile">
          ...
          <div
            className={`tile-value tone-${integrationTone(Boolean(brokerSnapshot?.connected ?? workbench.summary.brokerConnected), false, true)}`}
          >
            {brokerSnapshot?.connected
              ? copy[locale].labels.connected
              : copy[locale].labels.localMirror}
          </div>
          <div className="tile-sub">
            {locale === 'zh'
              ? '风险页现在优先消费后端 risk workbench 聚合快照'
              : 'The risk console now prioritizes the backend risk workbench snapshot.'}
          </div>
        </article>
        <article className="metric-tile">
          <div className="tile-label">{locale === 'zh' ? '集中度' : 'Concentration'}</div>
          <div className="tile-value">{workbench.summary.concentrationPct.toFixed(1)}%</div>
          <div className="tile-sub">
            {locale === 'zh' ? '按最大持仓 / 权益估算' : 'Largest position as a share of equity'}
          </div>
        </article>
        <article className="metric-tile">
          <div className="tile-label">{locale === 'zh' ? '合规提示' : 'Compliance Alerts'}</div>
          <div className={`tile-value tone-${workbench.summary.complianceAlerts ? 'warn' : 'ok'}`}>
            {workbench.summary.complianceAlerts}
          </div>
          <div className="tile-sub">
            {locale === 'zh'
              ? '策略、事件或 incident 中的合规/策略提示'
              : 'Policy or compliance-linked signals across risk events and incidents'}
          </div>
        </article>
        <article className="metric-tile">
          <div className="tile-label">{locale === 'zh' ? '应急动作' : 'Emergency Actions'}</div>
          <div className={`tile-value tone-${workbench.summary.emergencyActions ? 'risk' : 'ok'}`}>
            {workbench.summary.emergencyActions}
          </div>
          <div className="tile-sub">
            {locale === 'zh'
              ? 'blocked execution + risk-off 的合并姿态'
              : 'Combined blocked-execution and risk-off posture'}
          </div>
        </article>
        <article className="metric-tile">
          <div className="tile-label">{locale === 'zh' ? '调度关注' : 'Scheduler Attention'}</div>
          <div
            className={`tile-value tone-${workbench.summary.schedulerAttention ? 'warn' : 'ok'}`}
          >
            {workbench.summary.schedulerAttention}
          </div>
          <div className="tile-sub">
            {locale === 'zh'
              ? '会影响风控中台节奏的 scheduler attention'
              : 'Scheduler attention that can impact the risk middleware flow'}
          </div>
        </article>
      </section>
      ...
      <section className="panel-grid">
          ...
          <div className="panel-head">
            <div>
              <div className="panel-title">
                {locale === 'zh' ? 'Risk Workbench' : 'Risk Workbench'}
              </div>
              <div className="panel-copy">
                {locale === 'zh'
                  ? '把风险事件、执行复核、研究复核和 incident 收敛到同一份后端风险快照。'
                  : 'Aggregate risk events, execution review, research review, and incidents into one backend risk snapshot.'}
              </div>
            </div>
            <div className={`panel-badge badge-${riskTone(workbench.posture.status)}`}>
              {workbench.posture.status}
            </div>
          </div>
          <div className="status-stack">
            <div className="status-row">
              <span>{locale === 'zh' ? '最近策略动作' : 'Last Policy Action'}</span>
              <strong>{riskActionResult?.title || '--'}</strong>
            </div>
            <div className="status-row">
              <span>{locale === 'zh' ? '执行时间' : 'Executed At'}</span>
              <strong>{fmtDateTime(riskActionResult?.executedAt || '', locale)}</strong>
            </div>
          ...
          <div className="focus-list">
            {error ? (
              <div className="status-copy">
                {locale === 'zh'
                  ? `风险工作台加载失败：${error}`
                  : `Failed to load risk workbench: ${error}`}
              </div>
            ) : null}
            {loading ? (
              <div className="status-copy">
                {locale === 'zh' ? '正在同步风险工作台...' : 'Syncing risk workbench...'}
              </div>
            ) : null}
            {!loading
        ...
        <article className="panel">
          <div className="panel-head">
            <div>
              <div className="panel-title">
                {locale === 'zh' ? '服务端复核队列' : 'Server Review Queue'}
              </div>
              <div className="panel-copy">
                {locale === 'zh'
                  ? '优先查看真正由后端风险与执行边界驱动的待复核对象。'
                  : 'Prioritize the review items that are actually driven by backend risk and execution boundaries.'}
              </div>
            </div>
            <div
              className={`panel-badge ${workbench.summary.approvalRequired || workbench.summary.reviewBacktests || riskIncidentCount ? 'badge-warn' : 'badge-muted'}`}
            >
              {workbench.summary.approvalRequired +
                workbench.summary.reviewBacktests +
                riskIncidentCount}
            </div>
          </div>
          <div className="focus-list">
            {workbench.reviewQueue.executionPlans.slice(0, 3).map((plan) => (
              <InspectionSelectableRow
                ...
                metrics={[
                  {
                    label: locale === 'zh' ? '执行计划' : 'Execution Plan',
                ...
                actions={
                  <button
                    ...
                    onClick={() => openExecutionDetail(plan.id, plan.strategyId)}
                  >
              ...
              <InspectionSelectableRow
                ...
                metrics={[
                  ...
                  { label: locale === 'zh' ? 'Sharpe' : 'Sharpe', value: run.sharpe.toFixed(2) },
                  {
                    ...
                    value: `${run.maxDrawdownPct.toFixed(1)}%`,
                  },
                ]}
                actions={
                  <button
                    type="button"
                    className="inline-action"
                    onClick={() => openBacktestDetail(run.id, run.strategyId)}
                  >
            ...
            {workbench.reviewQueue.incidents.slice(0, 2).map((incident) => (
              <InspectionSelectableRow
                ...
                actions={
                  ...
                  >
                    {locale === 'zh' ? '打开排查台' : 'Open Investigation'}
                  </button>
                }
              />
            ))}
            {!workbench.reviewQueue.executionPlans.length &&
            ...
            !workbench.reviewQueue.incidents.length ? (
              <EmptyState
                ...
                message={
                  locale === 'zh'
                    ? '当前没有服务端复核队列项。'
                    : 'No server-backed review items are waiting right now.'
                }
              />
            ) : null}
          </div>
        ...
        <article className="panel">
          <div className="panel-head">
            <div>
              <div className="panel-title">{locale === 'zh' ? 'Risk Runbook' : 'Risk Runbook'}</div>
              <div className="panel-copy">
                {locale === 'zh'
                  ? '把 risk-off、review queue、合规提示和调度异常压成中台处置建议。'
                  : 'Turn risk-off, review queue, compliance alerts, and scheduler drift into operator-ready middleware actions.'}
              </div>
            </div>
            <div className={`panel-badge badge-${riskTone(workbench.posture.status)}`}>
              ...
              <div className="focus-row" key={item.key}>
                ...
                <div className="settings-chip-row">
                  <button
                    type="button"
                    className="inline-action"
                    onClick={() => focusRiskRunbook(item.key)}
                  >
                  ...
                  <button
                    type="button"
                    className="inline-action"
                    disabled={!canReviewRisk || riskActionBusy}
                    onClick={() => void executeRiskRunbook(item.key)}
                  >
                    {riskActionBusy
                      ? locale === 'zh'
                        ? '执行中...'
              ...
              <EmptyState
                ...
                message={
                  locale === 'zh'
                    ...
                    : 'No extra risk runbook actions are queued right now.'
                }
              />
            ) : null}
            {actionGuardNotice?.permission === 'risk:review' ? (
              <div className="status-copy">
                {formatActionGuardNotice(locale, actionGuardNotice)}
              </div>
            ) : null}
          </div>
        </article>
        <article className="panel">
          <div className="panel-head">
            <div>
              ...
              <div className="panel-copy">
                {locale === 'zh'
                  ? '把同一条风控问题在 risk 与 scheduler 两侧的上下文收成一份联动视图。'
                  : 'Collapse the shared risk and scheduler context for the same middleware issue into one linkage view.'}
              </div>
            </div>
            <div className={`panel-badge badge-${riskTone(workbench.linkage.posture.status)}`}>
              {workbench.linkage.summary.linkedRiskEvents +
                workbench.linkage.summary.linkedSchedulerTicks}
            </div>
          </div>
          <div className="focus-list">
            <div className="focus-row">
              <div className="focus-metric">
                <span>{locale === 'zh' ? '当前窗口' : 'Active Phase'}</span>
                <strong>{workbench.linkage.summary.activePhase || '--'}</strong>
              </div>
              <div className="focus-metric">
                <span>{locale === 'zh' ? 'Linked Risk' : 'Linked Risk'}</span>
                <strong>{workbench.linkage.summary.linkedRiskEvents}</strong>
              </div>
              <div className="focus-metric">
                <span>{locale === 'zh' ? 'Linked Scheduler' : 'Linked Scheduler'}</span>
                <strong>{workbench.linkage.summary.linkedSchedulerTicks}</strong>
              </div>
              <div className="focus-metric">
                <span>{locale === 'zh' ? 'Incidents' : 'Incidents'}</span>
                <strong>{workbench.linkage.summary.linkedIncidents}</strong>
              </div>
              <div className="focus-metric">
                <span>{locale === 'zh' ? 'Cycle Drift' : 'Cycle Drift'}</span>
                <strong>{workbench.linkage.summary.cycleAttention}</strong>
              </div>
            </div>
            <div className="status-copy">
              {workbench.linkage.posture.detail ||
                (locale === 'zh'
                  ? '当前还没有明显的 risk/scheduler 联动异常。'
            ...
            {workbench.linkage.runbook.map((item) => (
              <div className="focus-row" key={item.key}>
                ...
                <button
                  type="button"
                  className="inline-action"
                  onClick={() => focusRiskLinkageRunbook(item.key)}
                >
      ...
      <section className="panel-grid">
        <article className="panel">
          <div className="panel-head">
            <div>
              <div className="panel-title">
                {locale === 'zh' ? '风控中台队列' : 'Risk Middleware Queue'}
              </div>
              <div className="panel-copy">
                {locale === 'zh'
                  ? '把风控事件、执行复核、回测复核、incident 和 scheduler attention 收到同一个队列视角里。'
                  : 'Read risk events, execution review, backtest review, incidents, and scheduler attention through one middleware queue view.'}
              </div>
            </div>
            <div className="panel-badge badge-info">
              {workbench.reviewQueue.riskEvents.length +
                workbench.reviewQueue.executionPlans.length +
                workbench.reviewQueue.backtestRuns.length +
                workbench.reviewQueue.incidents.length +
          ...
          <div className="focus-list">
            <div className="focus-row">
              <div className="focus-metric">
                <span>{locale === 'zh' ? '风险事件' : 'Risk Events'}</span>
                <strong>{workbench.reviewQueue.riskEvents.length}</strong>
              </div>
              <div className="focus-metric">
                <span>{locale === 'zh' ? '执行复核' : 'Execution Review'}</span>
                <strong>{workbench.reviewQueue.executionPlans.length}</strong>
              </div>
              <div className="focus-metric">
                <span>{locale === 'zh' ? '回测复核' : 'Backtest Review'}</span>
                <strong>{workbench.reviewQueue.backtestRuns.length}</strong>
              </div>
              <div className="focus-metric">
                <span>{locale === 'zh' ? 'Incident' : 'Incidents'}</span>
                <strong>{workbench.reviewQueue.incidents.length}</strong>
              </div>
              <div className="focus-metric">
                <span>{locale === 'zh' ? '调度' : 'Scheduler'}</span>
                <strong>{workbench.reviewQueue.schedulerTicks.length}</strong>
              </div>
            </div>
              ...
              <InspectionSelectableRow
                ...
                actions={
                  <button
                    type="button"
                    className="inline-action"
                    disabled={selectedSchedulerTickId === item.id}
                    onClick={() => setSelectedSchedulerTickId(item.id)}
                  >
                    {selectedSchedulerTickId === item.id
                      ? locale === 'zh'
                        ? '已聚焦'
                        : 'Focused'
                      : locale === 'zh'
            ...
            {selectedSchedulerTick ? (
              <div className="status-stack">
                <div className="status-row">
                  ...
                  <strong>{selectedSchedulerTick.phase}</strong>
                </div>
                <div className="status-row">
                  <span>{locale === 'zh' ? '状态' : 'Status'}</span>
                  <strong>{selectedSchedulerTick.status}</strong>
                </div>
                <div className="status-row">
                  <span>{locale === 'zh' ? 'Worker' : 'Worker'}</span>
                  <strong>{selectedSchedulerTick.worker}</strong>
                </div>
              ...
              <EmptyState
                ...
                message={
                  locale === 'zh'
                    ...
                    : 'No scheduler attention items are queued right now.'
                }
              />
            ) : null}
          </div>
        ...
        <article className="panel">
          <div className="panel-head">
            <div>
              ...
              <div className="panel-copy">
                {locale === 'zh'
                  ? '保留当前 sandbox runtime 的实盘确认队列，用于前端交易沙盒演示。'
                  : 'Keep the current sandbox runtime approval gate for the frontend trading sandbox.'}
              </div>
            </div>
            <div
        ...
        <InspectionPanel
          ...
          copy={
            locale === 'zh'
              ? '从后端 risk workbench 中查看最近风险扫描事件，并把焦点固定到单条风控记录。'
              : 'Inspect the latest risk scan events from the backend risk workbench and pin focus on a single record.'
          }
                ...
                <InspectionSelectableRow
                  ...
                  actions={
                    <button
                      type="button"
                      className="inline-action"
                      disabled={selectedRiskEventId === item.id}
                      onClick={() => setSelectedRiskEventId(item.id)}
                    >
                      {selectedRiskEventId === item.id
                        ? locale === 'zh'
                          ? '已选中'
                          : 'Selected'
                        : locale === 'zh'
              ...
              {selectedRiskEvent ? (
                <div className="status-stack">
                  <div className="status-row">
                    ...
                    <strong>{selectedRiskEvent.source}</strong>
                  </div>
                  <div className="status-row">
                    <span>{locale === 'zh' ? '风险级别' : 'Risk Level'}</span>
                    <strong>{selectedRiskEvent.riskLevel}</strong>
                  </div>
                  <div className="status-row">
                    <span>{locale === 'zh' ? '周期' : 'Cycle'}</span>
                    <strong>{selectedRiskEvent.cycle}</strong>
                  </div>
      ...
      <section className="panel-grid">
        <article className="panel">
          <div className="panel-head">
            <div>
              <div className="panel-title">
                {locale === 'zh' ? '模拟盘持仓风险' : 'Paper Position Risk'}
              </div>
              <div className="panel-copy">
                {locale === 'zh'
                  ? '用现有持仓视图观察策略侧的仓位暴露。'
                  : 'Use the existing holdings view to inspect strategy-side position exposure.'}
              </div>
            </div>
        ...
        <article className="panel">
          <div className="panel-head">
            <div>
              <div className="panel-title">
                {locale === 'zh' ? '实盘持仓风险' : 'Live Position Risk'}
              </div>
              <div className="panel-copy">
                {locale === 'zh'
                  ? '这里现在直接消费 risk workbench 聚合回来的 broker 快照和 live 暴露。'
                  : 'This panel now consumes the broker snapshot and live exposure aggregated by the risk workbench.'}
              </div>
            </div>
