...
export function OverviewPage() {
  ...
  const openLiveOrders = state.accounts.live.orders.filter((order) =>
  ...
  const topSignals = state.stockStates
    ...
    .sort((a, b) => {
      const aBias =
        a.signal === 'HOLD' ? Math.abs(a.score - 50) : a.signal === 'BUY' ? a.score : 100 - a.score;
      const bBias =
        b.signal === 'HOLD' ? Math.abs(b.score - 50) : b.signal === 'BUY' ? b.score : 100 - b.score;
      return bBias - aBias;
    })
    .slice(0, 5);
  ...
  return (
    <>
      ...
      <section className={overviewHeroGrid}>
        <article className={`hero-card hero-card-primary ${overviewCommandCard}`}>
          <div className={overviewCommandFrame}>
            <div className="card-eyebrow">{copy[locale].overview.commandDeckEyebrow}</div>
            <div className={overviewCommandHead}>
              <div>
                <div className={overviewCommandTitle}>{copy[locale].slogans.commandCenter}</div>
                <div className={overviewCommandCopy}>
                  {translateRuntimeText(locale, state.routeCopy)}
                </div>
              </div>
              <div className={`status-chip status-chip-large tone-${riskTone(state.riskLevel)}`}>
                <span className="status-dot" aria-hidden="true" />
                {translateRiskLevel(locale, state.riskLevel)}
              </div>
            </div>
            <div className={overviewCommandCore}>
              ...
              <div className={overviewCommandAside}>
                <div className={overviewBriefCard}>
        ...
        <article className={`hero-card ${overviewKpiCard}`}>
          <div className="card-eyebrow">{copy[locale].overview.exposureEyebrow}</div>
          <div className={overviewKpiTitle}>{copy[locale].overview.exposureTitle}</div>
          ...
          <div className={overviewKpiGrid}>
            <div>
              <span>{copy[locale].labels.paper}</span>
              <strong>{paper.exposure.toFixed(1)}%</strong>
            </div>
            <div>
              <span>{copy[locale].labels.live}</span>
              <strong>{live.exposure.toFixed(1)}%</strong>
            </div>
          </div>
          <div className={overviewKpiNote}>{copy[locale].overview.exposureNote}</div>
        </article>

        <article className={`hero-card ${overviewKpiCard}`}>
          <div className="card-eyebrow">{copy[locale].overview.workflowEyebrow}</div>
          <div className={overviewKpiTitle}>{copy[locale].overview.workflowTitle}</div>
      ...
      <section className={`panel-grid panel-grid-terminal ${overviewDeskGrid}`}>
        <article className={`panel ${overviewPrimaryPanel}`}>
          <div className="panel-head">
            <div>
              <div className="panel-title">{copy[locale].terms.equityCurve}</div>
              <div className="panel-copy">
                {locale === 'zh'
                  ? '用一个主图盯盘总资产变化，并将信号、仓位和执行后果收敛到同一观察面。'
                  : 'Use one primary chart to track consolidated NAV, then read signal, exposure, and execution impact from the same desk.'}
              </div>
            </div>
            <div className="panel-badge badge-ok">{copy[locale].overview.liveDeskBadge}</div>
          </div>
          <div className={overviewPanelFlow}>
            <ChartCanvas kind="equity" />
            <div className={overviewInlineMetrics}>
              <div className={overviewInlineMetric}>
            ...
            <div className={overviewPrimaryNote}>
        ...
        <article className={`panel ${overviewSidePanel}`}>
          <div className="panel-head">
            <div>
              <div className="panel-title">{copy[locale].terms.executionSummary}</div>
              <div className="panel-copy">
                {locale === 'zh'
                  ? '把模式、风控、接入与最新动作压缩成一个侧边监控面板。'
                  : 'Compress mode, risk, connectivity, and the latest action into one desk-side monitor.'}
              </div>
            </div>
            <div className="panel-badge badge-muted">{copy[locale].overview.opsBadge}</div>
          </div>
          <div className="status-stack">
            <div className="panel-subtitle">{copy[locale].overview.autonomyControls}</div>
            <div className={overviewOpsCluster}>
              <button
                type="button"
                className="status-row status-row-button"
                onClick={() => goToSettings('switches')}
              >
                <span>{copy[locale].labels.autoTrade}</span>
                <strong className={`status-chip tone-${toggleTone(state.toggles.autoTrade)}`}>
                  {state.toggles.autoTrade
                    ? copy[locale].labels.enabled
                    : copy[locale].labels.disabled}
                </strong>
              </button>
              <button
                type="button"
              ...
              >
                ...
                <strong className={`status-chip tone-${toggleTone(state.toggles.liveTrade)}`}>
                  {state.toggles.liveTrade
                    ? copy[locale].labels.enabled
                    : copy[locale].labels.disabled}
                </strong>
              </button>
              ...
              >
                ...
                <strong className={`status-chip tone-${toggleTone(state.toggles.manualApproval)}`}>
                  {state.toggles.manualApproval
                    ? copy[locale].labels.enabled
                    : copy[locale].labels.disabled}
                </strong>
              </button>
            ...
            <div className={overviewOpsCluster}>
              <div className="status-row">
                ...
                <strong className={`status-chip tone-${monitoringTone(monitoringStatus?.status)}`}>
                  {monitoringLoading
                    ? locale === 'zh'
                      ? '加载中'
                      : 'Loading'
              ...
              <div className="status-row">
                <span>{copy[locale].labels.positions}</span>
              ...
              <div className="status-row">
                ...
                <strong>
                  {monitoringWorkerLag === null || monitoringWorkerLag === undefined
                    ? '--'
                    : `${monitoringWorkerLag}s`}
                </strong>
              </div>
              <div className="status-row">
            ...
            <div className={overviewOpsCluster}>
              ...
              >
                <span>{copy[locale].labels.marketState}</span>
                <strong
                  className={`status-chip tone-${integrationTone(marketConnected, marketDegraded)}`}
                >
                  {connectionLabel(locale, marketConnected, marketDegraded)}
                </strong>
              </button>
              <button
                type="button"
              ...
              >
                <span>{copy[locale].labels.brokerState}</span>
                <strong
                  className={`status-chip tone-${integrationTone(brokerConnected, false, true)}`}
                >
                  {connectionLabel(locale, brokerConnected, false, true)}
                </strong>
              </button>
            </div>
            ...
            <div className={overviewOpsCluster}>
              <div className="status-copy">
                {monitoringAlert
                  ? `${locale === 'zh' ? '当前告警' : 'Current alert'}: ${monitoringAlert.message}`
                  : locale === 'zh'
                    ? '当前没有新的监控告警。'
              ...
              <div className="status-copy">
                {monitoringUpdatedAt
                  ? `${locale === 'zh' ? '监控更新时间' : 'Monitoring updated'}: ${fmtDateTime(monitoringUpdatedAt, locale)}`
                  : locale === 'zh'
                    ? '监控摘要尚未返回。'
              ...
              <div className="status-copy">
                {state.activityLog[0]
                  ? `${locale === 'zh' ? '最新动作' : 'Latest action'}: ${translateRuntimeText(locale, state.activityLog[0].title)}`
                  : translateRuntimeText(locale, '当前没有新的执行记录。')}
              </div>
            </div>
          </div>
        </article>
      ...
      <section className={`panel-grid panel-grid-terminal-bottom ${overviewBlotterGrid}`}>
        <article className={`panel ${overviewBlotterCard}`}>
          <div className="panel-head">
            <div>
              <div className="panel-title">{copy[locale].terms.focusList}</div>
              <div className="panel-copy">
                {locale === 'zh'
                  ? '像交易员的 watchlist 一样，只保留最值得人工复核的机会。'
                  : 'Keep a trader-style watchlist with only the opportunities worth immediate review.'}
              </div>
            </div>
            <div className="panel-badge badge-muted">{copy[locale].overview.watchlistBadge}</div>
          </div>
          <div
            className={`focus-list focus-list-terminal ${overviewBlotterList} panel-body panel-body-sm`}
          >
        ...
        <article className={`panel ${overviewBlotterCard}`}>
          <div className="panel-head">
            <div>
              <div className="panel-title">{copy[locale].terms.recentOrders}</div>
              <div className="panel-copy">
                {locale === 'zh'
                  ? '首页保留精简 blotter，只看最新委托、账户归属和状态变化。'
                  : 'Keep a compact blotter on the home desk to review the latest order flow and status changes.'}
              </div>
            </div>
            <div className="panel-badge badge-info">
              {recentOrders.length || 0} {copy[locale].labels.orders}
            </div>
          </div>
          <div
            className={`focus-list focus-list-terminal ${overviewBlotterList} panel-body panel-body-sm`}
