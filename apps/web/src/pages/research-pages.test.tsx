...
describe('backtest lab panels', () => {
  it('BacktestEvidencePanel renders spec and result', () => {
    const spec = {
      id: 'spec-1',
      ...
      costModel: { model: 'bps' as const, bps: 5, label: 'Low cost BPS' },
      slippageModel: { model: 'volume' as const, volumeImpact: 0.1, label: 'Volume-based' },
      riskConstraints: {
    ...
    const html = renderToStaticMarkup(
      <BacktestEvidencePanel spec={spec} result={result} locale="en" />
    );
    expect(html).toContain('Backtest Evidence Panel');
    expect(html).toContain('abc123def456');
    expect(html).toContain('strat-v1');
    expect(html).toContain('ds-v3');
    expect(html).toContain('feat-v2');
    expect(html).toContain('SPX');
    expect(html).toContain('2020-01-01');
    expect(html).toContain('weekly');
    expect(html).toContain('Low cost BPS');
    expect(html).toContain('12.50%');
    expect(html).toContain('1.8500');
  });

  it('BacktestEvidencePanel shows empty state', () => {
    const html = renderToStaticMarkup(
      <BacktestEvidencePanel spec={null} result={null} locale="zh" />
    );
    expect(html).toContain('回测证据面板');
    expect(html).toContain('选择一条回测记录查看详情');
  });

  it('BacktestRobustnessPanel renders diagnostics and promotion eligibility', () => {
    const report = {
      parameterSensitivity: [
        { parameter: 'lookback', sharpeRange: [1.2, 1.8] as [number, number] },
      ],
      walkForwardSharpe: [1.5, 1.3, 1.6, 1.4],
      inSampleSharpe: 1.9,
      outOfSampleSharpe: 1.5,
      drawdownClusters: 2,
      turnoverExplosion: false,
      lowTradeCount: false,
      overfitRisk: 'low' as const,
    };

    const html = renderToStaticMarkup(<BacktestRobustnessPanel report={report} locale="en" />);
    expect(html).toContain('Robustness Diagnostics');
    expect(html).toContain('Eligible');
    expect(html).toContain('low');
    expect(html).toContain('1.9000');
    expect(html).toContain('1.5000');
    expect(html).toContain('lookback');
    expect(html).toContain('[1.2000, 1.8000]');
  });

  it('BacktestRobustnessPanel shows not eligible for high risk', () => {
    const report = {
      parameterSensitivity: [],
      walkForwardSharpe: [],
      inSampleSharpe: 3.5,
      ...
      drawdownClusters: 5,
      turnoverExplosion: true,
      lowTradeCount: true,
      overfitRisk: 'high' as const,
    };

    const html = renderToStaticMarkup(<BacktestRobustnessPanel report={report} locale="zh" />);
    expect(html).toContain('稳健性诊断');
    expect(html).toContain('不可晋级');
    expect(html).toContain('high');
  });

  it('BacktestAttributionPanel renders attribution and regime data', () => {
    const attributions = [
      { source: 'Momentum', contribution: 0.05, weight: 0.4 },
      { source: 'Value', contribution: 0.02, weight: 0.3 },
    ];
    ...
    expect(html).toContain('Attribution Analysis');
    expect(html).toContain('Momentum');
    expect(html).toContain('5.00%');
    expect(html).toContain('40.0%');
    expect(html).toContain('Regime Performance');
  ...
  it('BacktestAttributionPanel shows empty state', () => {
    const html = renderToStaticMarkup(
      <BacktestAttributionPanel attributions={[]} regimePerformance={[]} locale="zh" />
    );
    expect(html).toContain('归因分析');
    expect(html).toContain('暂无归因数据');
  });
});
