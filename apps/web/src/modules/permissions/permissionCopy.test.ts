...
describe('permission copy helpers', () => {
  ...
  it('formats read-only and disabled copy consistently', () => {
    expect(
      formatPermissionDisabled('zh', 'risk:review', '处理待复核回测', 'process review backtests')
    ).toContain('不能处理待复核回测');
    expect(
      formatPermissionReadOnly('en', 'strategy:write', '策略工作台', 'the strategy workspace')
    ).toContain('read-only');
  });

  it('formats API permission errors using the shared language', () => {
    const error = new ApiPermissionError(
      'missing required permission: account:write',
      403,
      'account:write'
    );
    expect(
  ...
  it('formats action-guard notices from action ids', () => {
    expect(
      formatActionGuardNotice('zh', {
        permission: 'execution:approve',
        action: 'cancel-live-order',
      })
    ).toContain('撤单');
    expect(
      formatActionGuardNotice('en', { permission: 'strategy:write', action: 'set-mode' })
    ).toContain('mode');
  });
});
