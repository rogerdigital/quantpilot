import type { ExecutionLedgerEntry, ExecutionWorkbenchResponse } from '../../../../../packages/shared-types/src/trading.ts';
...
export function getExecutionQueueFocusOptions(
  ...
  return [
    ...
    { key: 'approvals', label: locale === 'zh' ? '审批队列' : 'Approvals', count: queues?.approvals.length ?? 0 },
    { key: 'retryEligible', label: locale === 'zh' ? '重试队列' : 'Retry Queue', count: queues?.retryEligible.length ?? 0 },
    { key: 'compensationAutomation', label: locale === 'zh' ? '自动补偿' : 'Auto Comp', count: queues?.compensationAutomation.length ?? 0 },
    { key: 'compensation', label: locale === 'zh' ? '补偿队列' : 'Compensation', count: queues?.compensation.length ?? 0 },
    { key: 'incidents', label: locale === 'zh' ? 'Incident 队列' : 'Incidents', count: queues?.incidents.length ?? 0 },
    { key: 'activeRouting', label: locale === 'zh' ? '活跃路由' : 'Active Routing', count: queues?.activeRouting.length ?? 0 },
  ];
}
...
export function collectExecutionIncidentIds(entries: ExecutionLedgerEntry[], planIds: string[]): string[] {
