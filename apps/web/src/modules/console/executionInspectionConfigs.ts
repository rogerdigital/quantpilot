import type { BrokerAccountSnapshotRecord, ExecutionLedgerEntry, WorkflowRunRecord, WorkflowStepRecord } from '@shared-types/trading.ts';
...
export function getExecutionDetailInspectionConfig(
  ...
  return {
    ...
    emptyMessage: locale === 'zh' ? '当前没有可查看的执行计划。' : 'No execution plan is available for inspection.',
...
export function getExecutionAuditEventInspectionConfig(
  ...
  return {
    ...
    emptyMessage: !selectedEntry
      ? (locale === 'zh' ? '先从执行计划账本选择一条记录。' : 'Select an execution plan from the ledger first.')
      : !selectedAuditEvent
        ? (locale === 'zh' ? '当前执行计划还没有可钻取的审计事件。' : 'No execution audit event is available for inspection yet.')
        : null,
    metrics: [
      ...
      { label: locale === 'zh' ? '时间' : 'Time', value: selectedAuditEvent ? new Date(selectedAuditEvent.createdAt).toLocaleString(locale === 'zh' ? 'zh-CN' : 'en-US') : '--' },
    ],
  ...
  return {
    ...
    emptyMessage: !selectedEntry
      ...
      : executionDataLoading
        ? ''
        : !selectedWorkflow
          ? (locale === 'zh' ? '当前执行计划还没有可见的 workflow 详情。' : 'No workflow detail is available for the selected execution plan yet.')
          : null,
    metrics: [
  ...
  return {
    ...
    emptyMessage: !selectedEntry
      ...
      : !selectedWorkflowStep
        ? (locale === 'zh' ? '当前执行工作流还没有可定位的步骤。' : 'No execution workflow step is available for inspection yet.')
        : null,
    metrics: [
  ...
  return {
    ...
    emptyMessage: !selectedEntry
      ...
      : !selectedAccountSnapshot
        ? (locale === 'zh' ? '当前没有可用的 broker 快照。' : 'No broker snapshot is available for the selected execution plan.')
        : null,
  };
}
