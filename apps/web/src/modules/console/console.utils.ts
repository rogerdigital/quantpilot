...
export function translateSignal(locale: AppLocale, signal: string) {
  if (locale === 'zh')
    return (
      ({ BUY: '买入', HOLD: '持有', SELL: '卖出' } as Record<string, string>)[signal] || signal
    );
  return (
    (
      { BUY: 'BUY', HOLD: 'HOLD', SELL: 'SELL', 买入: 'BUY', 持有: 'HOLD', 卖出: 'SELL' } as Record<
        string,
        string
      >
    )[signal] || signal
...
export function translateRuntimeText(locale: AppLocale, text: string) {
  ...
  const replacements: Array<[RegExp, string]> = [
    ...
    [
      /未配置 VITE_MARKET_DATA_HTTP_URL，已回退到本地模拟行情。/g,
    ...
    [
      /HTTP 行情网关返回空结果，保留模拟行情。/g,
      'HTTP market gateway returned no data. Keeping simulated quotes.',
    ],
    [
      /HTTP 行情网关不可用，已回退到本地模拟行情。/g,
      'HTTP market gateway unavailable. Fell back to local simulated market data. ',
    ],
    ...
    [
      /Alpaca 行情不可用，已回退到本地模拟行情。/g,
      'Alpaca market data unavailable. Fell back to local simulated market data. ',
    ],
    [/使用本地模拟 broker 执行订单。/g, 'Using the local simulated broker to execute orders.'],
    [
      /使用本地模拟 broker 执行和回报。/g,
      'Using the local simulated broker for execution and fills.',
    ],
    [
      /本地模拟 broker 无远程撤单动作。/g,
      'The local simulated broker has no remote cancel action.',
    ],
    [
      /未配置 VITE_BROKER_HTTP_URL，当前仅使用本地实盘账户模拟链路。/g,
      'VITE_BROKER_HTTP_URL is not configured. Only the local live mirror is active.',
    ],
    [/Broker 网关已同步 (\d+) 笔订单。/g, 'Broker gateway synced $1 orders.'],
    [
      /Broker 网关不可用，保留本地镜像执行。/g,
      'Broker gateway unavailable. Keeping local mirrored execution. ',
    ],
    [
      /未配置 VITE_BROKER_HTTP_URL，无法同步远程账户。/g,
      'VITE_BROKER_HTTP_URL is not configured. Cannot sync the remote account.',
    ],
    [/Broker 网关状态同步成功。/g, 'Broker gateway state sync succeeded.'],
    [/Broker 状态同步失败。/g, 'Broker state sync failed. '],
    [
      /未配置 VITE_BROKER_HTTP_URL，无法撤单。/g,
      'VITE_BROKER_HTTP_URL is not configured. Cancel is unavailable.',
    ],
    ...
    [
      /Alpaca 账户、持仓和订单状态同步成功。/g,
      'Alpaca account, positions, and order state sync succeeded.',
    ],
