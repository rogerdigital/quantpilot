...
describe('research navigation deep links', () => {
  it('builds deep links without empty params', () => {
    expect(buildDeepLink('/execution', {
      plan: 'plan-1',
      strategy: 'strategy-1',
      run: '',
      timeline: undefined,
  ...
  it('reads deep link params from search params', () => {
    const params = new URLSearchParams('plan=plan-1&strategy=strategy-1&run=run-1&timeline=execution-plan-1&source=backtest&audit=audit-1&step=settlement_watch');
  ...
  it('navigates from strategy timeline to backtest detail with source context', () => {
    ...
    expect(navigate).toHaveBeenCalledWith('/backtest?run=run-1&strategy=strategy-1&timeline=run-run-1&source=strategies');
  });

  it('returns from execution to strategy timeline using current context', () => {
    const navigate = vi.fn();
    const navigation = useResearchNavigationContext(
      new URLSearchParams('plan=plan-1&strategy=strategy-1&timeline=execution-plan-1&source=strategies'),
      navigate,
    );

    navigation.returnToStrategyTimeline();

    expect(navigate).toHaveBeenCalledWith('/strategies?strategy=strategy-1&timeline=execution-plan-1');
  });
  ...
  it('navigates from backtest detail to execution detail with research context', () => {
    ...
    expect(navigate).toHaveBeenCalledWith('/execution?plan=plan-1&strategy=strategy-1&run=run-1&source=backtest');
  });
