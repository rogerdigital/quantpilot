...
export function getStrategyTerminalConfigs(options: {
  ...
  return {
    registry: {
      ...
      copy:
        locale === 'zh'
          ? '直接消费研究服务返回的策略目录，避免页面本地状态冒充注册表事实来源。'
          : 'Consume the backend strategy catalog directly instead of treating page-local runtime state as the registry source of truth.',
      badge: dataSourceBadge,
      badgeClassName: 'panel-badge badge-info',
      loading,
      isEmpty: visibleCount === 0,
      emptyMessage:
        locale === 'zh' ? '当前筛选条件下没有策略。' : 'No strategies match the current filter.',
      prelude: (
        <div className="status-stack">
          <div className="status-row">
            <span>{locale === 'zh' ? '活跃策略' : 'Active strategies'}</span>
            <strong>{activeCount}</strong>
          </div>
          <div className="status-row">
            <span>{locale === 'zh' ? '已归档策略' : 'Archived strategies'}</span>
    ...
    activity: {
      ...
      copy:
        locale === 'zh'
          ? '直接消费后端 audit records，查看策略注册、晋级、归档和恢复的最新留痕。'
          : 'Consume backend audit records directly to review recent registry saves, promotions, archives, and restores.',
      badge: 'AUDIT',
      ...
      emptyMessage:
        locale === 'zh'
          ? '当前筛选条件下没有策略操作历史。'
          : 'No strategy activity for the current filter.',
    },
  };
}
  ...
  return {
    ...
    queue: {
      ...
      emptyMessage:
        locale === 'zh'
          ? '当前筛选条件下没有回测记录'
          : 'No backtest runs match the current filter.',
    ...
    tasks: {
      ...
      isEmpty: taskCount === 0,
      emptyMessage:
        locale === 'zh' ? '当前还没有研究任务。' : 'No research tasks are available yet.',
      footer: (
        <div className="status-copy">
          {locale === 'zh'
    ...
    activity: {
      ...
      emptyMessage:
        locale === 'zh'
          ? '当前筛选条件下没有研究操作历史。'
          : 'No research activity for the current filter.',
    },
    workflows: {
      ...
      emptyMessage:
        locale === 'zh'
          ? '当前筛选条件下没有研究工作流。'
          : 'No research workflows for the current filter.',
    },
  };
}
