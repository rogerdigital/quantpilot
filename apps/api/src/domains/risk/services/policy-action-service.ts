...
function buildDescriptor(actionKey, workbench, targets) {
  ...
  const schedulerPhase =
    targets.schedulerTicks[0]?.phase || workbench.linkage.summary.activePhase || 'INTRADAY';
  ...
  if (actionKey === 'review-risk-off') {
    return {
      ...
      detail:
        'Risk-off events were moved into an active middleware review so the operator can explain the block and decide the next mitigation step.',
  ...
  if (actionKey === 'clear-review-queue') {
    return {
      ...
      detail:
        'Execution approvals and backtest review backlog were gathered into one risk-policy pass to keep the review queue from drifting.',
      riskLevel: 'REVIEW',
      schedulerPhase,
    };
  }
  if (actionKey === 'inspect-live-exposure') {
    return {
      level:
        workbench.summary.liveExposurePct >= 60 || workbench.summary.concentrationPct >= 20
          ? 'warn'
          : 'info',
      title: 'Inspect live exposure',
      detail:
        'Live exposure and concentration were reviewed against the current risk guardrails to confirm whether portfolio posture still fits the active policy.',
      riskLevel: 'REVIEW',
      schedulerPhase,
    };
  }
  if (actionKey === 'triage-risk-incidents') {
    return {
      ...
      detail:
        'Risk incidents were moved into an active triage pass so ownership, mitigation notes, and next actions are explicit on the middleware path.',
      riskLevel: 'REVIEW',
      schedulerPhase,
    };
  }
  ...
  if (actionKey === 'check-compliance-alerts') {
    return {
      ...
      detail:
        'Compliance-linked signals were gathered into a formal policy review so remaining constraints are visible before execution or promotion proceeds.',
      riskLevel: 'REVIEW',
      schedulerPhase,
    };
  }
  return {
    ...
    detail:
      'Blocked execution plans and risk-off posture were reviewed as one emergency-brake decision so the operator can decide whether to keep or release the block.',
    riskLevel: hasRiskOff ? 'RISK OFF' : 'REVIEW',
    schedulerPhase,
  };
}
