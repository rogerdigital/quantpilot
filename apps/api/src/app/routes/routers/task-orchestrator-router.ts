...
import {
  listActions,
  recordAction,
} from '../../../control-plane/task-orchestrator/services/action-service.js';
import {
  listCycles,
  recordCycleRun,
} from '../../../control-plane/task-orchestrator/services/cycle-service.js';
import {
  cancelWorkflow,
  getWorkflow,
  listWorkflows,
...
export async function handleTaskOrchestratorRoutes({
  ...
  if (req.method === 'GET' && reqUrl.pathname.startsWith('/api/task-orchestrator/workflows/')) {
    ...
    writeJson(
      res,
      workflow ? 200 : 404,
      workflow ? { ok: true, workflow } : { ok: false, message: 'workflow not found' }
    );
    return true;
  }
