...
export async function handleIncidentsRoutes({ req, reqUrl, res, readJsonBody, writeJson }) {
  ...
  if (req.method === 'GET' && reqUrl.pathname.startsWith('/api/incidents/')) {
    ...
    if (parts.length === 3) {
      ...
      writeJson(
        res,
        detail ? 200 : 404,
        detail ? { ok: true, ...detail } : { ok: false, message: 'incident not found' }
      );
      return true;
    }
  }
  ...
  if (
    req.method === 'POST' &&
    reqUrl.pathname.endsWith('/notes') &&
    ...
    writeJson(
      res,
      result ? 200 : 404,
      result ? { ok: true, ...result } : { ok: false, message: 'incident not found' }
    );
    return true;
  }
  ...
  if (
    req.method === 'POST' &&
    reqUrl.pathname.endsWith('/tasks') &&
    reqUrl.pathname.startsWith('/api/incidents/')
  ) {
    const incidentId = reqUrl.pathname.split('/').at(-2);
    const body = await readJsonBody(req);
    ...
    writeJson(
      res,
      task ? 200 : 404,
      task ? { ok: true, task } : { ok: false, message: 'incident not found' }
    );
    return true;
  }
  ...
  if (
    req.method === 'POST' &&
    reqUrl.pathname.includes('/tasks/') &&
    reqUrl.pathname.startsWith('/api/incidents/')
  ) {
    const parts = reqUrl.pathname.split('/').filter(Boolean);
    if (parts.length === 5) {
      ...
      writeJson(
        res,
        task ? 200 : 404,
        task ? { ok: true, task } : { ok: false, message: 'incident task not found' }
      );
      return true;
    }
  }
  ...
  if (req.method === 'POST' && reqUrl.pathname.startsWith('/api/incidents/')) {
    ...
    if (parts.length === 3) {
      ...
      writeJson(
        res,
        incident ? 200 : 404,
        incident ? { ok: true, incident } : { ok: false, message: 'incident not found' }
      );
      return true;
    }
  }
