...
export function handleDataRoutes({
  ...
  if (req.method === 'GET' && pathname === '/api/data/datasets') {
    const registry = createDatasetRegistry(getStore() as any);
  ...
  if (req.method === 'GET' && pathname === '/api/data/quality') {
    ...
    const reports = datasets
      .filter((d: any) => d.activeVersionId)
      .map((d: any) => registry.getQualityReport(d.activeVersionId))
      .filter(Boolean);
  ...
  if (req.method === 'GET' && pathname === '/api/data/features') {
    const registry = createFeatureRegistry(getStore() as any);
