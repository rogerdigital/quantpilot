  ...
  return (
    <>
      ...
      <section className="panel-grid">
        <article className="panel">
          <div className="panel-head"><div><div className="panel-title">{locale === 'zh' ? '策略研究入口' : 'Research Entry'}</div><div className="panel-copy">{locale === 'zh' ? '当前原型还没有真正的回测中心，这里先把研究入口和参数面板并到策略层。' : 'The prototype does not yet have a dedicated backtest center, so research entry stays inside the strategy layer for now.'}</div></div><div className="panel-badge badge-muted">RESEARCH</div></div>
